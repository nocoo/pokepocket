...
import { patchMgbaFilesystem } from './mgba-memory-filesystem.mjs';
...
export async function setupEmulator(options = {}) {
  ...
  const runtime = patchMgbaFilesystem(
    await readFile(path.join(source, 'dist/mgba.js'), 'utf8'),
    pkg.version,
  );
  ...
  await writeFile(path.join(target, 'mgba.js'), runtime);
  await copyFile(path.join(source, 'dist/mgba.wasm'), path.join(target, 'mgba.wasm'));
  await writeFile(
